 ...
 * For example, [10, 20, 30] corresponding to temperatures [50, 80, 100].
 * When we get a new temperature like T[i] = 90,
 * we will have [5, 30] as our list of indices (corresponding to temperatures [90, 100]).
 * The most basic structure that will satisfy our requirements is a stack,
 * where the top of the stack is the first value in the list,
 * and so on.
 *
 * Algorithm
 ...
 * T = [73, 74, 75, 71, 69, 72, 76, 73] in reverse order,
 * at the end of the loop (after we add T[i]).
 * For clarity, stack only contains indices i, but we will write the value of T[i] beside
