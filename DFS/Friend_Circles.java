 ...
 * In this graph, the node numbers represent the indices in the matrix M
 * and an edge exists between the nodes numbered ii and jj,
 * if there is a 1 at the corresponding M[i][j]M[i][j].
 *
 * In order to find the number of connected components in an undirected graph,
 * one of the simplest methods is to make use of
 ...
 * the node has already been visited while undergoing a Depth First Search from some node.
 *
