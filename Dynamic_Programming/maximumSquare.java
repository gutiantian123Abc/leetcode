 ...
 * Then we can immediately know that the first and last point can be a square of size 1 while the
 * two middle points cannot make any square, giving a size of 0.
 ...
 *     if matrix[i][j] = 1, res[i][j] = min(res[i - 1][j], res[i][j - 1], res[i - 1][j - 1]) + 1.
 */
